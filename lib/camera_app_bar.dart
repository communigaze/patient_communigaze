...
class CameraAppBar extends StatefulWidget with PreferredSizeWidget {
  ...
  // // new method to start the camera
  // void startCamera(BuildContext context) {
  //   final state = context.findAncestorStateOfType<_CameraAppBarState>();
...
class _CameraAppBarState extends State<CameraAppBar> {
  ...
  @override
  void didUpdateWidget(CameraAppBar oldWidget) {
  ...
  initializeCamera() async {
    ...
    await controller.initialize().then((_) {
      ...
      Stream<CameraImage> periodicImageStream = Stream<CameraImage>.periodic(
          Duration(milliseconds: (850 / frameRate).round()), (_) => img!);
  ...
  doFaceDetectionOnFrame() async {
    ...
    if (img != null) {
      ...
      for (Face f in faces) {
        ...
        // if mounted
        if (f.leftEyeOpenProbability! >= 0 && mounted) {
          if (f.leftEyeOpenProbability! > 0.5 &&
